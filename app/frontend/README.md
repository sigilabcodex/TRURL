...
4. Frontend renders:
   ...
   - contextual inspector with normalized read-only chapter metadata plus linked `character_ids`, `location_ids`, `timeline_ids`
...
The Write/Outliner toggle switches between the source editor and a read-only manuscript outliner inspired by longform writing tools such as Scrivener and Manuskript. The outliner summarizes chapters, approximate words, normalized status/type metadata, source/canon signals, tags, paths, and linked context counts from the already loaded workspace data. Selecting a row only selects the chapter; it does not edit, reorder, save, or call a backend endpoint. Chapter metadata normalization is documented in `docs/architecture/chapter-metadata.md`.
...
- no metadata editing; body-only saves still preserve frontmatter
