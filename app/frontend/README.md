...
4. Frontend renders:
   ...
   - real chapter list
   - read-only Outliner view for manuscript planning
   ...
   - contextual inspector with normalized read-only chapter metadata, advisory Metadata Health, and linked `character_ids`, `location_ids`, `timeline_ids`
...
The Write/Outliner toggle switches between the source editor and a read-only manuscript outliner inspired by longform writing tools such as Scrivener and Manuskript. The outliner summarizes chapters, approximate words, normalized status/type metadata, source/canon signals, tags, paths, and linked context counts from the already loaded workspace data. Selecting a row only selects the chapter; it does not edit, reorder, save, or call a backend endpoint. Chapter metadata normalization and advisory Metadata Health are documented in `docs/architecture/chapter-metadata.md`.
...
- read-only manuscript Outliner view with summary cards, metadata notes, and selectable chapter rows
- normalized read-only chapter metadata display in the Inspector
- advisory Metadata Health chips in the Inspector, Outliner, and Validation drawer
...
- no outliner drag-and-drop, reordering, metadata editing, or file writes
- no metadata editing; Metadata Health is advisory and body-only saves still preserve frontmatter
- no save/export blocking based on frontend Metadata Health yet
